//! Builder helper for constructing [`Scaler`] objects with proper metadata.
//!
//! Product operators use [`build_scaler`] to create a `Scaler` for each
...
use super::v1alpha1::{Scaler, ScalerSpec};
...
/// Constructs a [`Scaler`] with the required labels and owner reference.
///
...
// distinct Kubernetes metadata fields required on a Scaler. Grouping them
...
pub fn build_scaler(
    ...
    role: &str,
    role_group: &str,
    initial_replicas: u16,
    owner_ref: &OwnerReference,
    managed_by: &str,
) -> Result<Scaler, BuildScalerError> {
