...
//! `scaleTargetRef` always points at the correct [`Scaler`], and
...
use super::v1alpha1::Scaler;
use super::{ScalerState, ScalerStatus};
...
pub enum InitializeStatusError {
    ...
    #[snafu(display("failed to patch initial Scaler status"))]
    PatchStatus {
...
/// Build a [`CrossVersionObjectReference`] that points at a [`Scaler`].
///
...
/// # Parameters
///
/// - `scaler_name`: The `metadata.name` of the target [`Scaler`].
...
/// Patch a freshly created [`Scaler`]'s status to prevent scale-to-zero.
///
...
/// an `Idle` state so that the first reconcile sees the correct baseline.
///
/// # Parameters
///
/// - `client`: Kubernetes client for the status patch operation.
/// - `scaler`: The freshly created [`Scaler`] resource.
...
mod tests {
    ...
    fn build_hpa_overwrites_scale_target_ref() {
        ...
        let spec = hpa.spec;
        assert_eq!(spec.scale_target_ref.kind, "Scaler");
