//! Scaling lifecycle hooks for [`Scaler`](super::v1alpha1::Scaler).
//!
...
pub struct ScalingContext<'a> {
    ...
    /// and `PostScaling` it reflects the `previous_replicas` frozen inside the
    /// active state variant, so direction and ordinal calculations remain correct
    /// even after `status.replicas` has been updated to the target value.
